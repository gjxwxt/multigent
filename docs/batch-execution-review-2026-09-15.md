...
## 8. 执行审批记录（2026-09-16，用户授权"以功能为先，权衡后执行"）

> 背景：GPT 外审（round-16 两个问题）尚未返回；用户明确指示不再等待外审，
> 由执行方按"功能优先、架构代价明示"原则自行审批并执行，权衡记录在本节。
> 原则：所有架构都有代价——逐项写清接受什么、放弃什么、怎么回滚。

### 8.1 Batch C 试点——批准，范围切分为 C-1（本地）+ C-2（部署）

- **决策**：C-1 立即执行——本地 API 级全链路试点，用真实 git 仓库 fixture
  驱动 greenfield vNext 流水线，覆盖设计 §7 Batch C 六项检查单中的
  1/2/3/4/6，第 5 项（CI 与 QA 证据指向同一 SHA）以平台侧 checkpoint SHA
  断言作等价验证。C-2（真实 GitLab CI runner、双账号）待部署窗口执行。
- **接受的代价**：第 5 项的等价性依赖"CI 拉取的就是平台 checkpoint 的同一
  SHA"这一部署事实，C-1 只能证明平台侧 SHA 链路，真 CI 证据由 C-2 补齐。
- **回滚**：试点只新增测试与文档，无生产面；revert 对应 commit 即清。

### 8.2 Task 3.1 Change Run——批准开工，按 §4.1 v6 收紧后的设计执行

- **决策**：不再等待 GPT 复审放行；v6 已写入的两项 P0（clone 净化、跨进程
  锁 + proposal CAS）与三项 P1（验证顺序唯一化、V1 拒绝 dirty、记录脱敏
  限额）作为执行契约，按子特性独立 commit 推进。
- **接受的代价（逐项权衡）**：
  1. **V1 拒绝 dirty baseline**——发起前必须提交/暂存。代价是体验断点；
     换来 clone 基线 == 真实基线的确定性（alternate-index dirty 协议留档
     §4.1，待独立评审）。
  2. **同任务至多一个活跃 proposal**——串行化换冲突面清零；并行提案等真实
     需求出现再评审。
  3. **后端先行、UI 后置**——V1 端点返回结构化 JSON（diff、状态、验证
     结果），预览抽屉 UI 是纯展示层，不阻塞机制。
  4. **验证命令必须在沙箱容器内执行**（安全红线：禁止宿主直接 exec 项目
     argv）——复用 fixturesandbox 容器 generator；容器网络白名单仍未收口
     （既有妥协，继承 §4.1 记录）。
  5. **V1 不改写 preview copilot 现有直写路径**——Change Run 先作为独立
     机制落地（端点 + 状态机 + 隔离 apply），copilot 写路径切换为独立
     子特性（切换点单一、可单独回滚）。理由：一步切换会把新状态机的
     故障面直接压到现有预览体验上，分两步走每步都可独立验证。
- **回滚**：全部端点为新增路由，不下线既有端点；任一子特性 revert 后
  预览/审核链路不受影响。

### 8.3 round-16 评审简报——批准并入 Batch A/B/C-1 证据

- **决策**：简报追加 Batch A/B 交付与 C-1 试点结果，连同原有两个问题
  （SetXForwarded 覆盖、浏览器探测必要性）一并交 GPT 复审。
- **代价**：无（简报为 git-excluded 文件，不入库）。
