...
-1.8. **外部 QA 工具包（`qa-skill` / ming-qa）采纳评估定案**（方案 `docs/qa-skill-integration-plan.md`）：
   ...
   - **对第三方分工方案的三处复核修正**（均对着源码验证，不复述原方案）：① 平台**无**"按工作流步骤挂 skill"能力，绑定粒度只到 worker/team/role/project/workspace（`internal/ctxbuild/builder.go:15-175`；`WorkflowStep.Config` 被读的键仅 `designGate`/`joinPolicy`/`notify*`/`platform_gate`）；② 其硬编码产物根 `.qa-agent/`（`qa_agent.py:5303`、`:5706`）**撞我们自己的 QA 路径白名单**——目录白名单是精确段匹配，`.qa-agent` ≠ `qa`，故 `cases/`、`spec-tasks/`、`config/`、`reports/` 全被拒，而 `tests/api/*` 能过（**已实测**：`TestValidateQATouchedPaths*` 新增 5 条 `.qa-agent/*` 拒绝断言 + 1 条 `tests/api/*` 接受断言，PASS）；③ 其"查库断言"走 MySQL MCP，在本平台等于把 DB 凭据送进沙箱，与 `47d1844c` 刚收窄的凭据面冲突，归入任务 #2。
   - **顺带查出我们自己的一个洞**：`greenfield.go:316` 的 `e-atd-impl` 是无条件默认边——ATD 规格由 agent 产出后**直接进 implementation，无人确认过用例**；ADR `2026-09-18-delivery-pipeline-duality-and-atd-necessity.md` 只决定以结构化校验替代人工，那抓得住占位词与缺字段，抓不住"测错业务意图 / 漏资金权限路径"。
   - **采纳排序**：① 用例确认人工门禁（阶段 1，零外部依赖，改模板 + 一次重新实例化，greenfield 步骤 12→13）；② 只借 `completion-check.json`/`code-review-check.json`/`readiness-check.json` 的**契约**让平台复算 qa 结论，不引入 5300 行 `qa_agent.py`（阶段 2）；③ 跨任务回归语料库单独立项（阶段 4）。
   - **执行前置（到什么程度才可动手）**：阶段 1 只差一份 ADR 落盘；阶段 2 必须等 `docs/runbook-gate-acceptance-2026-09-19.md` §6 的 A–E 五格全部有实测记录（理由：阶段 2 是往同一条 step-complete 路径再加一个复算点，旧闸门未取证就叠加将无法归因）；阶段 3 需先关掉四个决策——路径冲突（**不通过放宽 `testpaths.go` 白名单解决**）、Playwright 浏览器（`docker/runtime-base/Dockerfile:78-87` 有 python3/rg/node 但无浏览器；改打 preview URL 还要先解决 30 分钟租期回收与签名 token）、`maxRepairLoops=5` 与 `maxReviewRounds=3` 的 cap 单一所有者、DB 凭据面。
   - **确定度**：`.qa-agent` 冲突与 `e-atd-impl` 无条件为实测/实读；`qa_agent.py` **未运行过**，其行为结论全部来自读其文档与脚本源码；preview 租期回收长 E2E 属推断未实测。
   - **连带项**：阶段 1 落地后需给上述 runbook 补 greenfield 的拓扑核对数字（现仅 unified 写了 13 步 / 18 边）。
