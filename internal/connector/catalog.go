...
func cloudflareProvider() Provider {
	return Provider{
		...
		Guides: []ProviderGuide{
			credentialGuide("Cloudflare API token", "Create a Cloudflare API token with only the account, zone, Workers, Pages, KV, R2, or DNS permissions your agents need. Prefer scoped tokens over global API keys.", "Cloudflare runtime inputs", "https://developers.cloudflare.com/fundamentals/api/get-started/create-token/"),
		},
		Enabled: true,
	}
}
