...
func softwareDelivery(locale string) entity.PlaybookTemplate {
	...
	return entity.PlaybookTemplate{
		...
		Roles: []entity.PlaybookRoleTemplate{
			...
			role("developer-agent", "engineering", "developer", text(locale, "Developer Agent", "开发 Agent"), text(locale, "Implements approved specs with clear changes, tests, and risk notes.", "按已审核方案实现代码，输出变更、测试和风险说明。"), []string{"task-management", "code-review", "git-pr-delivery"}),
		...
		Skills: []entity.PlaybookSkillTemplate{
			...
			skill("git-pr-delivery", text(locale, "Git PR Delivery", "Git 分支与 PR 交付"), text(locale, "Standard operating procedure for pushing branches, creating/updating PRs, and preparing review evidence.", "规范推送分支、创建/更新 PR 并准备审核证据的标准操作流程。")),
