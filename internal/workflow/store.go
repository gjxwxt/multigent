	...
	def := entity.WorkflowDefinition{
		...
		Steps: []entity.WorkflowStep{
			...
			},
			{
				...
				OutputFields: []entity.WorkflowField{{Name: "decision", Description: "approve or request_changes."}, {Name: "comments", Description: "Review comments and clarification notes."}},
		...
		Edges: []entity.WorkflowEdge{
			...
			edge("e-create-pr-review", "create_pr", "pr_review", "", nil, map[string]string{"approved_change": "$input.approved_change", "pr_url": "$output.pr_url", "pr_number": "$output.pr_number", "pr_diff_summary": "$output.pr_diff_summary", "preview_url": "$output.preview_url", "approved_technical_spec": "$input.approved_technical_spec"}, true),
			edge("e-pr-review-approve", "pr_review", "qa", "approved", cond("decision", "eq", "approve"), map[string]string{"approved_change": "$input.approved_change"}, false),
...
func softwareDeliveryTemplate(locale string) entity.WorkflowTemplate {
	...
	return entity.WorkflowTemplate{
		...
		Steps: []entity.WorkflowStep{
			...
			step("pr_review", "human_review", "prReviewTitle", "prReviewDesc", "owner-engineer", "amber", 2880, []entity.WorkflowField{field("approved_change", "approvedChangeInputField"), field("pr_url", "prURLField"), field("pr_number", "prNumberField"), field("pr_diff_summary", "prDiffSummaryField"), field("preview_url", "previewURLField"), field("approved_technical_spec", "approvedTechnicalSpecInputField")}, []entity.WorkflowField{field("decision", "decisionField"), field("comments", "commentsField")}),
			step("qa", "agent_task", "qaTitle", "qaDesc", "qa-agent", "rose", 3160, []entity.WorkflowField{field("approved_change", "approvedChangeInputField")}, []entity.WorkflowField{field("test_cases", "testCasesField"), field("test_report", "testReportField")}),
			step("qa_review", "human_review", "qaReviewTitle", "qaReviewDesc", "qa-owner", "amber", 3440, []entity.WorkflowField{field("test_report", "testReportReviewField")}, []entity.WorkflowField{field("decision", "decisionField"), field("comments", "commentsField"), field("release_candidate", "releaseCandidateField")}),
			step("release", "agent_task", "releaseTitle", "releaseDesc", "release-agent", "emerald", 3720, []entity.WorkflowField{field("release_candidate", "releaseCandidateInputField")}, []entity.WorkflowField{field("release_report", "releaseReportField")}),
		},
		Edges: []entity.WorkflowEdge{
			edge("e-req-to-review", "requirement_draft", "requirement_review", "", nil, nil, true),
			...
			edge("e-cleanup-create-pr", "changelog_cleanup", "create_pr", "", nil, map[string]string{"approved_change": "$input.approved_change", "approved_technical_spec": "$input.approved_technical_spec", "changelog": "$output.changelog", "branch_summary": "$output.branch_summary"}, true),
			edge("e-create-pr-review", "create_pr", "pr_review", "", nil, map[string]string{"approved_change": "$input.approved_change", "pr_url": "$output.pr_url", "pr_number": "$output.pr_number", "pr_diff_summary": "$output.pr_diff_summary", "preview_url": "$output.preview_url", "approved_technical_spec": "$input.approved_technical_spec"}, true),
			edge("e-pr-review-approve", "pr_review", "qa", text["approved"], cond("decision", "eq", "approve"), map[string]string{"approved_change": "$input.approved_change"}, false),
