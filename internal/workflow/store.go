...
func unifiedDeliveryPipelineTemplate(locale string) entity.WorkflowTemplate {
	...
	text := localizedTemplateText(locale, map[string]string{
		...
		"selfReviewField":     "Self-review verdict (pass / issues_fixed / escalate), evidence, and per-criterion results.",
		...
		"selfReviewDesc":           "独立审核 Agent——对照自己独立建立的预期基线裁决 diff，而非对照作者的自述。方法，严格按序：(1) 先读需求澄清与已确认范围，不打开 diff，独立写出预期基线：每条验收标准蕴含的行为、数据流、权限/风险边界、应当存在的测试切面。(2) 从 implementation 工件记下 base SHA 与 HEAD SHA，只读该区间的 diff，逐条标准对照基线。(3) 亲自重跑构建、测试与 lint——没有亲手跑过的检查视为 unverified，不能支撑 approve。(4) 检查是否混入范围外改动。输出契约：self_review 使用 pass / issues_fixed / escalate 结论词表，附逐条标准结果与证据；escalation_case 是结构化问题清单（每条：违反什么契约、文件:行号、影响、最小修复、缺失的验证）。存在 P0/严重缺陷且 review_rounds < 3 时打回编码重做；达到三轮则升级人工审核并附遗留问题案卷。严禁未经实际验证就通过。独立性边界：不要以「我会用另一种架构实现」本身作为问题——那不是缺陷；不要读取或信任作者的思考链，只裁决 diff 与其可观察行为。环境型故障升级规则：若失败由环境因素（镜像源不可达、网络受限、runner 能力/架构不匹配、依赖服务缺失）导致且同一环境问题已持续 2 轮以上，禁止在代码层面继续空转——升级人工审核并附环境诊断证据，由人工决定环境处置方案。",
	...
	return templateFromParts("unified-delivery-pipeline", text["name"], text["description"], locale, "clarify",
		...
		[]entity.WorkflowEdge{
			...
			edge("e-self-pass", "agent_self_review", "ci_ready_gate", text["approved"], cond("self_review", "neq", "escalate"), nil, false),
