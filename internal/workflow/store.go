...
func (s *Store) SeedDefaults() error {
	...
	def := entity.WorkflowDefinition{
		...
		Steps: []entity.WorkflowStep{
			...
			{
				ID: "push_to_gitlab", Type: "agent_task", Title: "Push to GitLab or Remote",
				Description:  "Push the verified branch to GitLab / remote repository and record the remote branch or PR URL.",
				ActorRole:    "developer-agent",
				InputFields:  []entity.WorkflowField{{Name: "branch_summary", Description: "Branch name, commit summary, and delivery evidence."}, {Name: "approved_change", Description: "Code artifact approved for testing."}},
				OutputFields: []entity.WorkflowField{{Name: "gitlab_url", Description: "GitLab or remote repository branch/PR URL."}, {Name: "push_summary", Description: "Push output summary and remote references."}},
				Position:     entity.WorkflowPosition{X: 3160, Y: 80},
				Config:       map[string]string{"color": "emerald"},
			},
			{
		...
		Edges: []entity.WorkflowEdge{
			...
			edge("e-pr-review-push", "pr_review", "push_to_gitlab", "approve and push", cond("decision", "in", "approve_push,push"), map[string]string{"branch_summary": "$input.branch_summary", "approved_change": "$input.approved_change"}, false),
			edge("e-push-to-qa", "push_to_gitlab", "qa", "", nil, map[string]string{"approved_change": "$input.approved_change"}, true),
			edge("e-pr-review-approve", "pr_review", "qa", "approved", cond("decision", "in", "approve,approve_local"), map[string]string{"approved_change": "$input.approved_change"}, false),
...
func softwareDeliveryTemplate(locale string) entity.WorkflowTemplate {
	...
	return entity.WorkflowTemplate{
		...
		Steps: []entity.WorkflowStep{
			...
			step("push_to_gitlab", "agent_task", "pushToGitlabTitle", "pushToGitlabDesc", "developer-agent", "emerald", 3160, []entity.WorkflowField{field("branch_summary", "branchSummaryField"), field("approved_change", "approvedChangeInputField")}, []entity.WorkflowField{field("gitlab_url", "gitlabURLField"), field("push_summary", "pushSummaryField")}),
		...
		Edges: []entity.WorkflowEdge{
			...
			edge("e-cleanup-create-pr", "changelog_cleanup", "create_pr", "", nil, map[string]string{"approved_change": "$input.approved_change", "approved_technical_spec": "$input.approved_technical_spec", "changelog": "$output.changelog", "branch_summary": "$output.branch_summary"}, true),
			edge("e-create-pr-review", "create_pr", "pr_review", "", nil, map[string]string{"approved_change": "$input.approved_change", "pr_url": "$output.pr_url", "pr_number": "$output.pr_number", "pr_diff_summary": "$output.pr_diff_summary", "preview_url": "$output.preview_url", "approved_technical_spec": "$input.approved_technical_spec", "branch_summary": "$input.branch_summary"}, true),
			edge("e-pr-review-push", "pr_review", "push_to_gitlab", text["approveAndPush"], cond("decision", "in", "approve_push,push"), map[string]string{"branch_summary": "$input.branch_summary", "approved_change": "$input.approved_change"}, false),
			edge("e-push-to-qa", "push_to_gitlab", "qa", "", nil, map[string]string{"approved_change": "$input.approved_change"}, true),
			edge("e-pr-review-approve", "pr_review", "qa", text["approved"], cond("decision", "in", "approve,approve_local"), map[string]string{"approved_change": "$input.approved_change"}, false),
