...
func unifiedDeliveryPipelineTemplate(locale string) entity.WorkflowTemplate {
	...
	text := localizedTemplateText(locale, map[string]string{
		...
		"selfReviewDesc":      "Independent reviewer agent: re-run build, tests, and lint yourself; verify every acceptance criterion; check for changes outside the approved scope. Recycle P0/severe defects to coding while review_rounds < 3; otherwise escalate to human review with an outstanding-issues case file. Never approve unverified code. Environment-failure escalation: when the failure stems from environmental factors (unreachable mirrors, restricted network, runner capability/arch mismatch, missing dependency services) and the same environment issue has persisted for 2+ rounds, do not keep spinning in code changes — escalate to human review with the environment diagnostics and let a human decide the remediation.",
		...
		"selfReviewDesc":           "独立审核 Agent：亲自重跑构建、测试与 lint；逐条核对验收标准；检查是否混入范围外改动。存在 P0/严重缺陷且 review_rounds < 3 时打回编码重做；达到三轮则升级人工审核并附遗留问题案卷。严禁未经实际验证就通过。环境型故障升级规则：若失败由环境因素（镜像源不可达、网络受限、runner 能力/架构不匹配、依赖服务缺失）导致且同一环境问题已持续 2 轮以上，禁止在代码层面继续空转——升级人工审核并附环境诊断证据，由人工决定环境处置方案。",
