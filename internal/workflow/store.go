...
func unifiedDeliveryPipelineTemplate(locale string) entity.WorkflowTemplate {
	...
	text := localizedTemplateText(locale, map[string]string{
		...
		"releaseDesc":         "Tag the release candidate, push it, and trigger CI/CD on a best-effort basis (record none plus the reason if the connection lacks pipeline permission). Then verify post-release signals: health endpoint, error logs, and a rollback plan. Only ship what QA sign-off approved.",
		...
		"decisionField":       "approve or request_changes.",
		"commentsField":       "Verdict record: why approved/request_changes, adjudication results, ownership markers for issues introduced elsewhere, and dispositions of manual walkthrough items. This field is NOT passed downstream.",
		"approvedScopeField":  "Frozen delivery contract (implement/self-review/QA are judged against it). Must include: approved scope with adjudicated parameter decisions, itemized acceptance criteria, non-goals. Only this field is passed to the implement node on approval — adjudications written in comments are invisible downstream.",
		...
		"releaseTitle":        "发布与观察",
		"releaseDesc":         "为发布候选打 Tag 并推送，尽力触发 CI/CD（连接缺少流水线权限时如实填 none 并说明原因）。随后验证发布后信号：健康端点、错误日志与回滚预案。只发运 QA 准出批准的候选。",
		"requestField":        "原始需求、缺陷报告或上下文。",
		"clarifiedField":      "澄清后的范围，含验收标准与非目标说明。",
		"decisionField":       "approve 或 request_changes。",
		"commentsField":       "裁决记录：通过/打回的理由、疑点裁决结果、非本任务问题的归属标记、人工走查项的处理决定。注意：此字段不会传递给下游节点。",
		"approvedScopeField":  "冻结的交付契约（实现/自审/QA 均以此为准）。必须包含：批准的范围（含裁决后的参数形态）、逐条验收标准、非目标。通过后仅此字段传给实现节点——写在审核意见里的裁决下游不可见，必须折叠进来。",
	...
	return templateFromParts("unified-delivery-pipeline", text["name"], text["description"], locale, "clarify",
		...
		[]entity.WorkflowEdge{
			...
			edge("e-clarify-approved", "clarify_review", "implement", text["approved"], cond("decision", "eq", "approve"), map[string]string{"approved_scope": "$output.approved_scope"}, false),
