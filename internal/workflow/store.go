...
func unifiedDeliveryPipelineTemplate(locale string) entity.WorkflowTemplate {
	...
	text := localizedTemplateText(locale, map[string]string{
		...
		"prDesc":              "Push the task branch and create or update the same PR (reuse previous_pr on rework) with changelog, diff summary, tests, and preview evidence.",
		...
		"prDesc":              "推送任务分支并创建或更新同一个 PR（返工时复用 previous_pr），内容包含变更日志、Diff 摘要、测试与预览证据。",
