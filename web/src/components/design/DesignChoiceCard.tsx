...
import { Check, Layers, ShieldAlert, Sparkles } from 'lucide-react'
import { cn } from '../../lib/cn'

export type DesignSource = 'existing' | 'generate' | 'waive'
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected
          ? isWaive
            ? 'border-amber-500 bg-amber-50/70 ring-1 ring-amber-500 dark:border-amber-500 dark:bg-amber-950/40'
    ...
    >
      <span className="flex items-center gap-2">
        ...
        <span className="flex items-center gap-1.5 text-sm font-medium text-neutral-900 dark:text-zinc-100">
          {value === 'existing' ? (
            <Layers className="size-4 text-neutral-400 dark:text-zinc-500" />
          ) : value === 'generate' ? (
